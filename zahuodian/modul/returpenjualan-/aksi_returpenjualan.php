...
if ($module=='returpenjualan' AND $act=='delete'){
 ...
 while ( $r=mysql_fetch_array($query)){
  ...
  mysql_query($delete);
  Echo "UPDATE `trans_sales_invoice` SET `status_retur`='0' WHERE `id_invoice`='$no_invoice'";
...
$kode_rjb=cari_kode();
if (check_si($_POST[no_invoice])==0) {
  input_only_log("INSERT INTO trans_retur_penjualan(
                                                kode_rjb, 
                                                no_invoice, 
                                                id_customer, 
                                                jenis_retur, 
    ...
    $query_header = "INSERT INTO trans_retur_penjualan_detail(
                                                ...
                                                tgl_update)
                                                VALUES ";
 $itemCount1 = count($_POST["id"]);
 input_jurnal_umum('91','36','','1',$_POST["returgrandtotal"],$kode_rjb,$_POST['tgl_rjb'],$_POST['customer'],'customer');
   for($i=0; $i<$itemCount1;$i++) {
    if (!empty($_POST["qty_convert"][$i])) {
              $split_satuan=explode('-', $_POST["jenis_satuan"][$i]);
              if(!empty($kode_rjb) || !empty($_POST["id_barang"][$i]) || !empty($_POST["id_gudang"][$i]) || !empty($_POST["jml-rjb"][$i])  || !empty($_POST["harga-rjb"][$i]) || !empty($_POST["total-rjb"][$i])) {
                  $itemValues++;
                  if($querydetail!="") {
                    $querydetail .= ",";
                  }
                  ...
                  BS($_POST["qty_convert"][$i],$module,'tambah');
                    ...
                    $t =mysql_fetch_array(mysql_query("SELECT `stok_sekarang` FROM `stok` WHERE `id_barang`='".$_POST["id_barang"][$i]."' and `id_gudang`='". $_POST["id_gudang"][$i]."'"));
                        input_only_log("update stok set stok_sekarang=". ($t["stok_sekarang"] +$_POST["qty_convert"][$i])." WHERE id_barang = '".$_POST["id_barang"][$i]."' AND id_gudang =  '". $_POST["id_gudang"][$i]."'",$module);
                         echo 'aa'.$_POST['jenis_retur'].'<br>';
                }
                  $querydetail .= "('" . $kode_rjb . "', '" . $_POST["id_barang"][$i] . "','" . $_POST["id_gudang"][$i] . "', '" . $_POST["qty_si"][$i]. "', '" . $_POST["qty_si_satuan"][$i] . "', '" . $_POST["qty_si_convert"][$i]   . "', '" . $_POST["jml-rjb"][$i] . "', '" . $split_satuan[1] . "', '" .$_POST["qty_convert"][$i] . "', '" .$_POST["total-rjb"][$i] . "', '" .$_POST["harga-rjb"][$i] . "', '" . $_SESSION["namauser"]. "', now() )";
                  ################################### Maslah Jurnal ##############################
  ...
  input_only_log($query_header.$querydetail,$module);



input_data("UPDATE trans_sales_invoice set status_retur=1 where id_invoice='$_POST[no_invoice]' ",$module);
}else{
