...
<table id="table_penotaan" class="table table-hover table-bordered" cellspacing="0">
    ...
    <td colspan="2" rowspan="5"> <br><button class="btn btn-success"  type="submit"  name="save" value="Save" style="float:left;margin-left:20px;">Save </button> 
          <a class="btn btn-warning" type="button" href="media.php?module=tukanganpenotaan" style="float:left;margin-left:10px;">Batal</a>
          </td>
  ...
  case "edit":
  $id=$_GET['id'];
$query=mysql_query("SELECT * FROM `trans_totalan_tukang` a,supplier b WHERE a.`id_supplier`=b.`id_supplier` and `id`='".$id."'");
$r=mysql_fetch_array($query);
?>
<h2><b>Edit</b> Penotaan Tukang</h2><hr>
<form method='post' action='modul/tukanganpenotaan/aksi_tukanganpenotaan.php?module=tukanganpenotaan&act=edit'>
     <div class='table-responsive'>
      <table class='table table-hover' border=0 id='tambah'>
  <tr>
        <td>No Penotaan</td>
        <td><strong>:</strong></td>
        <td><strong><?= $r[no_totalan_tukang]?></strong>
          <input type="hidden" name='no_totalan_tukang' value="<?= $r[no_totalan_tukang] ?>" ></td>
        <td>Tgl. Penotaan</td>
        <td><strong>:</strong></td>
        <td><input class="form-control datetimepicker" value=" <?php echo $r[tgl_totalan] ?>" id="tgl_NTT" name="tgl_NTT" required></td>
  </tr>
  <tr>
        <td>Supplier</td> 
        <td><strong>:</strong></td>
    ...
    <tr>
      <td>Nominal      </td> 
        <td><strong>:</strong></td>
        <td><?= format_rupiah($r['nominal_hutang']) ?></td>
      <td>No. Nota Tukang</td> 
        <td><strong>:</strong></td>
        <td><?= $r['nota_tukang'] ?>
           <input type="hidden" id='nonota_terima_tukang' name='nota_tukang' value="<?= $r['nota_tukang'] ?>" >
        </td>
    </tr>
<tr>
  <td>
...
<table id="table_penotaan" class="table table-hover table-bordered" cellspacing="0">
      ...
      <th>Aksi</th>
        ...
        <tbody id='body_table_penotaan'>
        <?php $query="SELECT *,a.harga as harga_NTT  FROM `trans_totalan_tukang_detail` a, barang b,stok_tukang c WHERE a.id_barang=b.id_barang and a.id_barang=c.id_barang and c.id_supplier='".$r[id_supplier]."' and `no_totalan_tukang`='".$r['no_totalan_tukang']."'";
        $noz=1000;
...
echo '  <tr >
    <td>'.$rst['kode_barang'].' <br> '.$rst['nama_barang'].'
     ...
     <td>'.($rst['stok_tukang']+$rst['convert']).' '.$rst['satuan1'].' 
        <input type="hidden" name="stok_tukang[]" value="'.($rst['stok_tukang']+$rst['convert']).'" id="stok_tukang-'.$noz.'"  readonly />
    </td>
  ...
  <td>
    ...
    echo'
      ...
      <input name="qty_satuan[]" id="qty_satuan-'.$noz.'" value="'.$rst['kali'].'" class="NTT"  type="hidden" readonly="readonly" />
       <input  class="NTT numberhit" type="text" value="'.$rst['convert'].'" name="convert[]" id="convert-'.$noz.'"  readonly="readonly" />
    </td>
     <td>
  <input type="text" name="harga[]"  value="'.(floor($rst['harga_NTT'])).'" id="harga_NTT-'.$noz.'"  class="NTT totalNTT  form-control numberhit"  />
     </td>
 <td>
     
       <input type="text" name="jumlah[]" id="jumlah_NTT-'.$noz.'" value="'.$rst['jumlah'].'"  class="NTT form-control numberhit" />
    </td>
<td colspan="3"><input type="text" class="sub_total  form-control numberhit" name="total[]" value="'.$rst['total'].'" id="total_NTT-'.$noz.'"   readonly/></td>';?>
...
$noz++;
    ...
    <td colspan="4" style="text-align:right;" ><p><b>Total    :  </b></p></td>
    <td colspan="1">
    <input name="nominal_hutang" type="hidden"  class="NTT2 form-control numberhit"  id="nominal_hutang" value="<?= $r['nominal_hutang'] ?>"  readonly><strong><?= format_rupiah($r['nominal_hutang']) ?></strong></td>
    <td width="30px"><center><strong><b><span>  -   </span></b></strong></center></td>
    <td ><input name="penotaantotal" type="text"  class="NTT2 form-control numberhit"  id="penotaantotal"  readonly></td>
  </tr>
  <tr>
  ...
  <tr>
     <td colspan="6" style="text-align:right;"><p> Disc (%) <input value="<?= $r[discper] ?> " name="penotaandiscpersen" type="text" id="penotaandiscpersen" style="width:2em;"  class="NTT2 form-control numberhit" > | (Rp) </p></td>
    <td colspan="1" style="nowrap:nowrap;"><input value="<?= $r[disc] ?> " name="penotaandiscnominal" type="text" id="penotaandiscnominal"   class="NTT2 form-control numberhit"  ></td>
  </tr>
  <tr>
    <td colspan="6" style="text-align:right;"><p> Ppn (%) <input value="<?= $r[ppnper] ?> " name="returppnpersen" type="text" id="penotaanpersenppn"  style="width:2em;" class="NTT2 form-control numberhit" > | (Rp) </p></td>
    <td colspan="1"  style="nowrap:nowrap;"><input value="<?= $r[ppn] ?> " name="penotaanppnnominal" type="text" id="penotaanppnnominal"   class="NTT2 form-control numberhit"  ></td>
  </tr>
  <tr>
     <td colspan="6" style="text-align:right;"><b>Grand total </b></td>
    <td colspan="1"><b><input name="grandtotalpenotaan" type="text" id="grandtotalpenotaan"  readonly="readonly"  class="NTT2 form-control numberhit" ></b></td>
...
function deleteRow(r) {
    ...
    document.getElementById("table_penotaan").deleteRow(r);
           disc();
           ppn();
           totalpenotaan()
           grandtotalpenotaan();
