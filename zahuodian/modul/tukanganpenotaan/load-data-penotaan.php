...
$columns = array(
	...
	array( 'db' => '`u`.`tanggal`',  'dt' => 6 , 'field' => 'tanggal' ),
    	array(
	       ...
	       $edit=explode('-', $d);
	       if ($edit[1]=='0') {
	       	return '<a  class="btn-sm btn-warning" href="?module=tukanganpenotaan&act=edit&id='.$edit[0].'"><span class="glyphicon glyphicon-list" aria-hidden="true"></span></a>';
	       }
...
$joinQuery = "FROM (SELECT concat(a.id,'-',a.status_terbayar) as edit ,a.id,a.`no_totalan_tukang`,`id_terima_tukang`,`nota_tukang`,`nama_supplier`,`nominal_totalan`,date_format(`tgl_totalan`,'%d %M %Y') as tanggal FROM `trans_totalan_tukang` a, `trans_totalan_tukang_detail` b, supplier c WHERE a.`no_totalan_tukang`=b.`no_totalan_tukang` and a.`id_supplier`=c.`id_supplier` and a.is_void='0'  group by a.`no_totalan_tukang` order by `id` desc) AS `u`  ";
