...
// memanggil file config.php
 ...
 include "../../lib/input.php";
  ...
  if (isset($_GET['titipansupplierglobal'])) { // ##########################	QUERY GLOBAL
...
$columns = array(
	...
	array( 'db'        => 'CONCAT(`tb`.`id_bayarbeli`," # ",`tb`.`nominal_alokasi`)', 'dt'        => 9,  'field' => 'id_bayarbeli', 'as' => 'id_bayarbeli',
	       'formatter' => function( $d ) {
	       	 $jenisaksi = explode(" # ", $d);
	       	 if($jenisaksi[1] == '0'){
	       	$s =  '<div class="row">
	       	<a class="btn-sm btn-warning" href="?module=titipansupplier&act=alokasi&id='.$d.'"><span class="glyphicon glyphicon-list" aria-hidden="true"></span></a>
	       <a class="btn-sm btn-success" onclick="editTitipan('.$jenisaksi['0'].')" ><span class="glyphicon glyphicon-edit" aria-hidden="true"></span></a>
	       </div>';
	    }
	       else {
	       	$s =  '<a class="btn-sm btn-warning" href="?module=titipansupplier&act=alokasi&id='.$d.'"><span class="glyphicon glyphicon-list" aria-hidden="true"></span></a>';
	       }
	       return $s;
    })
);
