...
switch($_GET['act']){
  // Tampil Modul
  default:
$judul = "Laporan Pembelian Group By Supplier";
$desk =  "Laporan transaksi Pembelian dari setiap <b>Supplier</b>";
...
$judul = "Kartu Pembelian Detail Pernota";
$desk =  "Laporan transaksi pembelian dari supplier $r[nama_supplier] per <b>Nota</b>";
...
echo "<div class='row'>
  ...
  break;
case 'po':
  echo "<h2><b>Detail</b> Purchase Order</h2>
...
echo"
  ...
  <tr>
    <td colspan=2 style=text-align:right;><p> Disc (%) $r[discper] | (Rp) </p></td>
    <td colspan=2 style=nowrap:nowrap; align=right>".format_rupiah($r[disc])."</td>
  </tr>
  <tr>
    <td colspan=2 style=text-align:right;><p> Ppn (%) $r[ppnper] | (Rp) </p></td>
  ...
  break;
case 'lpb':
echo "<h2><b>Detail</b> Laporan Barang Masuk</h2>
   <form method='post' action='$aksi?module=purchaseorder&act=update'>
     <div class='table-responsive'>
      <table class='table table-hover' border=0 id=tambah>
  <tr>
    <td>No LBM</td>
    <td>";
  $query=mysql_query("SELECT * FROM `trans_lpb` t, supplier s   WHERE t.id_supplier=s.id_supplier and id_lpb = '$_GET[id]' order by id desc limit 1 ");
 $r=mysql_fetch_array($query);
     echo "  
  $r[id_lpb]";
   echo "</td>
...
$r[id_pur_order]
  ...
  <tr>
   <td>Tanggal barang diterima</td>
    <td>".date('d-m-Y',strtotime($r[tgl_lpb]))."</td>
   <td> No Nota Supplier : </td>
    <td>$r[no_nota_supplier]</td>
  </tr>
  <tr>
   <td>Supplier</td>
...
$tampiltable=mysql_query("SELECT *,concat(qty,'-',qty_satuan) as jumlah_dlm_po, concat(qty_diterima,'-',qty_diterima_satuan) as trima FROM
  `trans_lpb_detail` d,gudang g WHERE d.id_gudang=g.id_gudang and id_pur_order = '$r[id_pur_order]' order by kode_barang_po,id");
 $no=1;
...
echo "<h2><b> Detail </b>Purchase Invoice</h2>
     <div class='table-responsive'>
      <table class='table table-hover' border=0 id=tambah>
  <tr>
    <td>No Pembayaran</td>
...
while ($rst = mysql_fetch_array($tampiltable)){
  ...
  <tr>
   ...
   <td align=center>
    ...
    <td>
       ".format_rupiah($rst['harga_po'])."
    </td>
     <td align=right>
       $rst[total_po]
    </td>
     <td align=center>
       ".convt_satuan($rst['qty_pi_convert'],$rst['id_barang'])." [$rst[qty_pi_convert]]
    </td>
     <td>
