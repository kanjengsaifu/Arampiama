...
switch($_GET['act']){
  ...
  default:
        ...
        $desk = "Modul Yang digunakan untuk meretur barang kepada supplier";
        $button="<a href='?module=returpembelian&act=tambah' class='btn btn-primary' >Tambah Retur Pembelian <span class='glyphicon glyphicon-plus' aria-hidden='true'></span></a><span class='success'>";
        headerDeskripsi($judul,$desk,$button);
        if(isset($message)) { echo $message; }

echo '</span>
<div class="table-responsive">
<table id="po" class="table table-hover table-bordered" cellspacing="0">
        <thead>
  <tr style="background-color:#F5F5F5;">
    <th  id="tablenumber">No</th>
      ...
      <th width="10%"  colspan=2>Aksi</th>
        ...
        <tbody>';
      $sql="SELECT * FROM `trans_retur_pembelian` trp left join supplier s on (s.id_supplier=trp.id_supplier) WHERE trp.is_void= '0' limit 50";
...
<tr>
           <td>$no</td>
            <td>$r[kode_rbb]</td>
            <td>".tgl_indo($r['tgl_rbb'])."</td>
            <td>$r[kode_supplier] - $r[nama_supplier]</td>
             <td>";
             if ($r['jenis_retur']==1){
              echo "Deposit";
             }
             elseif ($r['jenis_retur']==2){
              echo "Tukar Guling";
             }

             echo "</td>
                <td>".format_rupiah($r[grandtotal_retur])."</td>
            ";
 if ($r['bukti']==''){
    echo " 
    <td>
    <a 
    href='modul/returpembelian/cetak.php?id=$r[id]' 
    ...
    <td>
    <a 
    href='$aksi?module=returpembelian&act=hapus&id=$r[id]&kode=$r[kode_rbb]' 
