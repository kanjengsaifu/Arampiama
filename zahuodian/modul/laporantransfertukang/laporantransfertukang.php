...
$aksi="modul/laporantransfertukang/laporantransfertukang.php";
switch($_GET['act']){

// Tampil Modul
  default:
    echo "<h2>Laporan Transfer Bahan Baku Antar Tukang</h2>

<div class='table-responsive'>
	<table class='table table-hover'>
		<form method='post' action='?module=laporantransfertukang'>
...
if (isset($_POST['save'])||isset($_GET['t1'])) {
	...
	<div class='table-responsive'>
	    <table class='table table-bordered'>
	        ...
	        <tbody>";
	        $sql = "SELECT tt.*, b.nama_barang, s.nama_supplier as `tukang_asal`,s2.nama_supplier as `tukang_tujuan` FROM `transfer_tukang` tt, barang b,supplier s, supplier s2 where tt.id_barang = b.id_barang and s.id_supplier = tt.id_supplier_dari and s2.id_supplier = tt.id_supplier_pada AND tt.is_void = 0";
	        if (isset($_POST['save'])) {
	        	$sql.=" AND tgl_transfer_tukang BETWEEN '$_POST[tgl1]' AND '$_POST[tgl2]'";
	        } else {
	        	$sql.=" AND tgl_transfer_tukang BETWEEN '$_GET[t1]' AND '$_GET[t2]'";
	        }
	        $sql.=" ORDER BY id_trasnfer_tukang";
	        //echo $sql;
	        ...
	        while ($rows = mysql_fetch_array($query)) {
	        	...
	        	<tr>
              <td>$no</td>
	        		<td>$rows[no_transfer_tukang]</td>
	        		<td>$rows[no_surat_jalan]</td>
	        		<td>$rows[nama_barang]</td>
              <td>".number_format($rows['harga'])."</td>
              <td>".number_format($rows['jumlah'])."</td>
              <td>$rows[tukang_asal]</td>
              <td>$rows[tukang_tujuan]</td>
	        		<td>".tgl_indo($rows['tgl_transfer_tukang'])."</td>
	        	</tr>
	        	";
          $no++;
	        }
