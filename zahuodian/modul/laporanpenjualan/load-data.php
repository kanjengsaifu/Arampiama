...
$columns = array(
	...
	array('db' => '`u`.`id_sales_order`', 'dt' => 1, 'field' => 'id_sales_order', 
		'formatter' => function($d){
			$link = "<a href = '?module=penjualannota&act=detailso&id=$d' title='Detail SO' target='_blank'>".$d."</a>";
			return $link;
		}),
	array('db' => '`u`.`id_lkb`', 'dt' => 2, 'field' => 'id_lkb' ,
		'formatter' => function($d){
			$link = "<a href = '?module=penjualannota&act=detaillkb&id=$d' title='Detail LKB' target='_blank'>$d</a>";
			return $link;
		}),
	array('db' => '`u`.`id_invoice`', 'dt' => 3, 'field' => 'id_invoice',
		'formatter' => function($d){
			$link = "<a href = '?module=penjualannota&act=detailsi&id=$d' title='Detail SI' target = '_blank'>$d</a>";
			return $link;
		}),
...
t.id,t.id_invoice,lkb.id_lkb,tso.id_sales_order,nama_customer,telp_customer,nama_sales,t.is_void
...
and hpp.id_invoice=t.id_invoice) as u";
	...
	$extraWhere .=  " AND `u`.`tgl_update` >= '".$periode_awal."'";
	}elseif( (!empty($periode_awal)) && (!empty($periode_akhir))){
	$extraWhere .= " AND (u.tgl_update BETWEEN  '".$periode_awal."' AND '".$periode_akhir."') order by u.tgl_update asc";
	}
