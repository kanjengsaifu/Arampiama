...
if ($_POST['jenis-laporan-sales']=='0') {
  if ($id_sales==0) {
  echo '
...
while ($t=mysql_fetch_array($tquery)) {
 ...
 <tr>
<th align=right >Nama Sales <strong>:<strong></th>
<th align=left colspan="1">'.$t['nama_sales'].'</th>
<th align=right >Tanggal Invoice <strong>:<strong></th>
<th align=left colspan="1">'.$t['tanggal'].'</th>
<th align=right >Nomor Invoice <strong>:<strong></th>
<th align=left colspan="1">'.$t['id_invoice'].'</th>
<th align=right >Nama Customer <strong>:<strong></th>
...
<table style="width: 100%;">
<tr>
<td>Tanggal Dibuat : '.tgl_indo(date("Y/m/d")).' - '.date("h:i:s a") .'</td>
 <td align=right><span>Total : <span> '.format_jumlah($jumlah).'</td>
 </table>
<footer>';
}elseif ($_POST['jenis-laporan-sales']=='1') {
  echo '
<table style="width: 100%;">
<tr>
<th colspan="4"  style="font-size: 26px;">LAPORAN KOMISI SALES</th>
</tr>
<tr >
<th  colspan="4" >UD. MELATI</th>
</tr>
...
<tbody>";
$query="SELECT * FROM `trans_sales_invoice`tsi, `trans_bayarjual_detail` tbd, customer c,`trans_bayarjual_header` tbh
 WHERE c.id_customer=tsi.id_customer and `id_invoice`=nota_invoice and tbh.bukti_bayarjual=tbd.bukti_bayarjual";
$result=mysql_query($query);
while ($r=mysql_fetch_array($result)) {
  echo "
<tr>
<td>".$r[nama_customer]."</td>
<td>".$r[nota_invoice]."</td>
<td>".$r[bukti_bayarjual]."</td>
<td>".$r[nominal_alokasi_detail_jual]."</td>
<td>";
if ($r[jatuh_tempo_jual]!='0000-00-00') {
  echo $r[jatuh_tempo_jual];
}
echo "
</td>
<td>";
if ($r[tgl_giro_cair]!='0000-00-00') {
  echo $r[tgl_giro_cair];
}
echo "</td>
</tr>
  ";
